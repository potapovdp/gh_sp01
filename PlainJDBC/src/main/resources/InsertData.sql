insert into USER (id, LOGIN, PASSWORD, HASH_PASS, ROLE_TYPE) values (1, 'user1', 'user1pass', 'ghjghjgjgjh', 'User');
insert into USER (id, LOGIN, PASSWORD, HASH_PASS, ROLE_TYPE) values (2, 'user2', 'user2pass', 'dfgdfgfdgdfg', 'User');
insert into USER (id, LOGIN, PASSWORD, HASH_PASS, ROLE_TYPE) values (3, 'admin', 'admin1pass', 'ggdfgfdgfd', 'Admin');
...
/* simple join sample */
SELECT distinct * FROM USER U, SESSION S, PROJECT P, TASK T
WHERE U.ID = P.USER_ID AND P.ID = T.PROJECT_ID AND U.ID = S.USER_ID AND U.ID=1